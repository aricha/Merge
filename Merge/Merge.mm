...
static id _logos_method$FloatingHeader$CKTranscriptHeaderView$initWithFrame$isPhoneTranscript$displayLoadPrevious$isGroupMessage$(CKTranscriptHeaderView* self, SEL _cmd, CGRect frame, BOOL transcript, BOOL previous, BOOL message) {
	...
	BOOL isLandscape = (width - 320.0 >= (480.0 - 320.0) / 2); 
...
static void _logos_method$FloatingHeader$CKTranscriptHeaderView$addSubview$(CKTranscriptHeaderView* self, SEL _cmd, UIView * subview) {
		...
		[[(UIPlacardButton *)subview actionsForTarget:self forControlEvent:UIControlEventTouchUpInside] containsObject:@"_clipboardButtonClicked:"])
	{
		...
		_logos_orig$FloatingHeader$CKTranscriptHeaderView$addSubview$(self, _cmd, subview);
	}
}
...
static UIPlacardButton * _logos_method$FloatingHeader$CKTranscriptHeaderView$copyToClipboardButton(CKTranscriptHeaderView* self, SEL _cmd) {
    return objc_getAssociatedObject(self, MGBiteSMSCopyToClipboardButtonKey);
}



static void _logos_method$FloatingHeader$CKTranscriptHeaderView$setCopyToClipboardButton$(CKTranscriptHeaderView* self, SEL _cmd, UIPlacardButton * copyToClipboardButton) {
...
#ifdef DEBUG
	NSLog(@"-[<CKTranscriptController: %p> _handlePreferredServiceChangedNotification:%@]", self, notification);
#endif
...
static CKServiceView * _logos_method$_ungrouped$CKTranscriptController$pendingServiceViewIfVisible(CKTranscriptController* self, SEL _cmd) {
...
static BOOL _logos_method$_ungrouped$CKAggregateConversation$isCombinedConversation(CKAggregateConversation* self, SEL _cmd) {
    ...
    for (CKSubConversation *conversation in self.subConversations) {
		if (conversation.recipients.count == 1) {
			...
			else if (![prevGroupID isEqualToString:conversation.groupID] ) {
